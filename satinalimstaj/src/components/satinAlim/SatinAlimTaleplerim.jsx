...
const SatinAlim = () => {
  ...
  const handleSorgulaClick = async () => {
    try {
      ...
      const response = await axios.post('https://localhost:7092/api/v1/satinAlim/SatinAlim/PersonelTalepListele', payload);
      ...
      <table>
        ...
        <tbody>
          {talepListesi?.map((item, index) => (
            <tr key={index} style={{ textAlign:'center', backgroundColor: item.talepHizmetListe.length > 0 ? "#adebbe" : "#eef3ad" }}>
